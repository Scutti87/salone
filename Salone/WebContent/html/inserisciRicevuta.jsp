...
<body>
	...
	<form class="form-ins-mod-eli" id="inserisci"
		action="${pageContext.request.contextPath}/inserisciRicevuta?vengo=aggiungi&clienteId=${clienteId}"
		method="post" onsubmit="return mostraPopup()">
		<h1 align="center">Inserisci Ricevuta</h1>
		...
		<!-- Altri campi del form (Servizi, Cliente, ecc.) -->

		<label>Servizi</label>
		<div class="servizi-container">
			<label class="container"> <input type="checkbox"
				name="servizi[]" value="PIEGA" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('PIEGA')}">checked</c:if>> Piega
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="TAGLIO" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('TAGLIO')}">checked</c:if>> Taglio
				<div class="checkmark"></div>
			</label><label class="container"> <input type="checkbox"
				name="servizi[]" value="COLORERIT" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('COLORERIT')}">checked</c:if>> Colore Rit
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="COLORECOMPL" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('COLORECOMPL')}">checked</c:if>> Colore Compl
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="TONER" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('TONER')}">checked</c:if>> Toner
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="MECHES" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('MECHES')}">checked</c:if>> Meches
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="BAYALAGE" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('BAYALAGE')}">checked</c:if>> Bayalage
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="SHATUSH" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('SHATUSH')}">checked</c:if>> Shatush
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="PERMAN" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('PERMAN')}">checked</c:if>> Perman
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="LISCIANTE" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('LISCIANTE')}">checked</c:if>> Lisciante
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="TRATT" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('TRATT')}">checked</c:if>> Tratt
				<div class="checkmark"></div>
			</label> <label class="container"> <input type="checkbox"
				name="servizi[]" value="TAGLIOU" onchange="calcolaTotale()" <c:if test="${servizi != null && servizi.contains('TAGLIOU')}">checked</c:if>> Taglio Uomo
				<div class="checkmark"></div>
			</label>
		</div>
		...
		<br> <label
			...
			name="totale" value="" pattern="[0-9]+" readonly>

		<!-- Bottone di invio del form -->
		<button type="submit" onclick="return validateFormCliente()">Inserisci</button>
	</form>
	...
	function confermaSconto() {
	    calcolaTotale();
	    chiudiPopup();
	    document.getElementById('inserisci').submit(); // Invia il form
	}	
	</script>
